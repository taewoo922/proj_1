...
//            else if (cmd.startsWith("회원목록")) {
//                if ( members.size() == 0) {
//                    System.out.println("회원이 없습니다.");
//                    continue;
//                }
//
//                String searchKeyword = cmd.substring("회원목록".length()).trim();
//                //사용자가 검색어를 입력하면 searchKeyword에 담는다
//
//
//                List<Member> forListMemebers = members;
//
//                if (searchKeyword.length() > 0) {         //검색어가 있다면
//                    forListMemebers = new ArrayList<>();  //새로운 리스트를 만들어 준다
//
//                    for ( Member member : members) {                //그리고 반복문을 통해 기존 리스트를 돌고
//                        if (member.loginId.contains(searchKeyword) ) {  //키워드를 포함한 타이틀이 있다면
//                            forListMemebers.add(member);              //그 article을 새로운 리스트에 추가한다.
//                        }
//                    }
//
//                    if ( members.size() == 0) {
...
//                }
//
//                System.out.println(" id | 이름 ");      //만약 검색어를 입력하지 않고 그냥 검색어 목록만
//                for (int i = forListMemebers.size() -1; i >= 0; i--) {  //입력하면 모든 목록을 보여준다.
//                    Member member = forListMemebers.get(i);
//
